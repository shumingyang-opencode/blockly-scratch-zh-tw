...
把**視覺化程式設計**（積木邏輯、事件、變數、專案）與 **Blockly × Scratch 雙工具**做成系統化深度教學。每個單元都附可照做的**專案範例**與**雙工具對照**，讓「積木邏輯」變成「真的作品與真的程式碼」。
...
| L3 | 14 | 從積木到文字程式（JS/Python） |
| L3 | 15 | Blockly 技術面入門（自訂積木） |
